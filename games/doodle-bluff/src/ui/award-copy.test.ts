...
describe("doodleBluffAwardCopy", () => {
  it("pen-of-the-people: singular and plural", () => {
    expect(doodleBluffAwardCopy(award("pen-of-the-people", 1), en)).toEqual({
      title: "Pen of the people",
      detail: "1 person found your real titles",
    });
    expect(doodleBluffAwardCopy(award("pen-of-the-people", 4), en)).toEqual({
      title: "Pen of the people",
  ...
  it("master-forger: singular and plural", () => {
    expect(doodleBluffAwardCopy(award("master-forger", 1), en)).toEqual({
      title: "Master forger",
      detail: "Fooled 1 person with fake titles",
    });
    expect(doodleBluffAwardCopy(award("master-forger", 3), en)).toEqual({
      title: "Master forger",
  ...
  it("sharp-eye: singular and plural", () => {
    expect(doodleBluffAwardCopy(award("sharp-eye", 1), en)).toEqual({
      title: "Sharp eye",
      detail: "Found the real title 1 time",
    });
    expect(doodleBluffAwardCopy(award("sharp-eye", 5), en)).toEqual({
      title: "Sharp eye",
  ...
  it("abstract-artist: singular and plural", () => {
    expect(doodleBluffAwardCopy(award("abstract-artist", 1), en)).toEqual({
      title: "Abstract artist",
      detail: "Fooled 1 person — nobody found the truth",
    });
    expect(doodleBluffAwardCopy(award("abstract-artist", 2), en)).toEqual({
      title: "Abstract artist",
  ...
  it("returns null for an unknown id", () => {
    expect(doodleBluffAwardCopy(award("mystery", 1), en)).toBeNull();
  });
});
